...
# Helpers: validation for latitude / longitude
...
# Helpers: validation for forecast dates
...
get_forecast <- function(latitude,
  ...
  if (time_resolution == "hourly") {	
    # Making each variable a column in the dataframe
    weather_data <- as.data.frame(lapply(data_json$hourly, function(x) unlist(x)))
    # Making time a datetime column in the dataframe
    ...
    # Making time a date column in the dataframe
...
get_nearest_mountains <- function(latitude,
  ...
  if (!is.numeric(num_mountains) ||
      length(num_mountains) != 1 ||
      is.na(num_mountains) ||
      num_mountains < 1) {
  ...
  if (!is.numeric(prominence_threshold) ||
      length(prominence_threshold) != 1 ||
      is.na(prominence_threshold) ||
      prominence_threshold < 0) {
    stop("prominence_threshold must be a single number >= 0.")
  }
  
  # Error handling: elevation_threshold is 0 or more
  if (!is.numeric(elevation_threshold) ||
      length(elevation_threshold) != 1 ||
      is.na(elevation_threshold) ||
      elevation_threshold < 0) {
    stop("elevation_threshold must be a single number >= 0.")
  }
  
  # Load mountains data from package extdata
  ...
  # If no mountains match the thresholds
  ...
  # Converting m to km for distance
  distances <- round(distHaversine(mountain_coords, input_coord) / 1000, 1)
...
forecast_mountains <- function(mountains, 
  ...
  # Error handling: If no rowname, create generic ones
